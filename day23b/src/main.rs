...
fn h(state: State) -> i32 {
    state.hallway.into_iter().enumerate().filter_map(|(n, spot)| {
        ...
        room.into_iter().enumerate().filter_map(move |(m, spot)| {
            spot.and_then(|pod| {
                if pod != n as u8 {
                    Some(((ROOM_COLS[pod as usize] - ROOM_COLS[n]).abs() + m as i32 + 2) * COSTS[pod as usize])
                } else {
                    None
                }
            })
...
fn main() {
    ...
    for (n, line) in input.lines().skip(2).enumerate() {
        for (m, c) in line.chars().filter(|c| c.is_ascii_uppercase()).enumerate() {
            start.rooms[m][n * 3] = Some(c as u8 - 'A' as u8);
        }
    }

    start.rooms[0][1] = Some(3);
    start.rooms[0][2] = Some(3);
    start.rooms[1][1] = Some(2);
    start.rooms[1][2] = Some(1);
    start.rooms[2][1] = Some(1);
    start.rooms[2][2] = Some(0);
    start.rooms[3][1] = Some(0);
    start.rooms[3][2] = Some(2);
        ...
        for (n, room) in state.rooms.into_iter().enumerate() {
            if let Some(y) = if room.into_iter().any(|pod| pod.map_or(false, |pod| pod != n as u8)) {
                ...
                for i in (0..).map(|i| ROOM_HALLWAYS[n].1 + i).take_while(|&i| {
                    ...
                    let new_cost = cost + (
                        y as i32 + 1 + (ROOM_COLS[n] - HALLWAY_COLS[i]).abs()
        ...
        for (x, spot) in state.hallway.into_iter().enumerate() {
            if let Some(pod) = spot {
                ...
                if (room.into_iter().all(|spot| {
                    ...
                    let new_cost = cost + (
                        y as i32 + 1 + (ROOM_COLS[pod as usize] - HALLWAY_COLS[x as usize]).abs()
                    ) * COSTS[pod as usize];
                    let f_score = new_cost + h(new_state);
