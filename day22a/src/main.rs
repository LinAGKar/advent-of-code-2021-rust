...
fn main() {
    ...
    let src_cuboids: Vec<_> = input.lines().filter_map(|line| {
        ...
        if (0..3).all(|i| {
            let res = cuboid[2 * i] <= 50 && cuboid[2 * i + 1] >= -50;
            cuboid[2 * i] = std::cmp::max(cuboid[2 * i], -50);
            cuboid[2 * i + 1] = std::cmp::min(cuboid[2 * i + 1], 50);
            res
        }) {
            Some((on, cuboid))
        } else {
    ...
    println!("{}", enabled_cuboids.into_iter().map(|cuboid| {
        (0..3).map(|i| cuboid[2 * i + 1] - cuboid[2 * i] + 1).product::<i32>()
    }).sum::<i32>());
}
